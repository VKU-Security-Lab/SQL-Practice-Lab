...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (strpos(strtoupper($query), 'SELECT') !== 0) {
        $error = "Câu truy vấn không hợp lệ (Chỉ được sử dụng <code>SELECT</code>)";
    } else {
        try {
...
<body>
    <h1>Challenge 1</h1>
    <p><b>Mô tả thử thách: </b>Hãy lấy thông tin của người dùng có tên là: <b>John Wick</b></p>
    ...
    <div id="flagDiv">
        ...
        <form action="checkFlag.php" method="post" id="formFlag">
            ...
            <input type="text" id="idChall" name="idChall" hidden value="1">
            <button type="submit">Gửi</button>
        </form>
    </div>

    <form action="challenge1.php" method="post" id="formQuery">
            ...
            $("#formFlag").submit(function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response) {
                            localStorage.setItem("chall1", response);
                            alert("Flag chính xác");
                            window.location.href = "challenge2.php";
                        } else {
                            alert("Sai rồi");
                        }
                    }
